...
export default function PerkHistoryScreen(): React.JSX.Element {
  ...
  const historyItems = useMemo<PerkHistoryItem[]>(() => {
    return partnerRedemptions.map((redemption) => {
      const partner = partners.find(
        (item) => item.id === redemption.partner_id
      );
  ...
  }, [i18n.language, partnerRedemptions, partners, t]);
        ...
        {partnerRedemptionsLoading ? (
          ...
          <View className="gap-3">
            {filteredItems.map((item) => {
              ...
              return (
                <View
                  className="flex-row items-center rounded-2xl border border-border bg-card px-3 py-3 dark:border-dark-border dark:bg-dark-bg-card"
                  key={item.id}
