const common = {
  ...
  legal: {
    ...
    accountDeletion: {
      title: 'Esco Beach Club 계정 삭제',
      intro:
        '이 공개 페이지는 Esco Beach Club 계정 삭제를 요청하는 방법, 30일 복원 기간 동안 일어나는 일, 삭제되는 데이터, 그리고 법적·보안상 이유로 삭제 후 보관될 수 있는 기록을 설명합니다.',
      sections: {
        overview: {
          title: '한눈에 보기',
          body1:
            '앱 안의 프로필 > 계정 삭제에서 삭제를 시작하거나, 앱 내 흐름에 접근할 수 없으면 지원팀에 연락해 주세요.',
          body2:
            '삭제가 예약되면 계정은 30일 복원 기간에 들어가며, 그 안에 다시 로그인하면 삭제를 취소할 수 있습니다.',
          body3:
            '삭제가 최종 완료되면 계정 프로필, 회원 정보, 저장한 이벤트, 예약 내역, 프라이빗 이벤트 요청 내역, 회원 전용 혜택이 삭제됩니다.',
          body4:
            '법적 준수, 사기 방지, 보안 유지, 회계 처리, 또는 삭제 요청 처리 기록이 필요한 경우에만 제한된 기록을 보관할 수 있습니다.',
        },
        request: {
          title: '삭제 요청 방법',
          body1:
            '앱을 열고 프로필 > 계정 삭제로 이동한 뒤, 경고 내용을 확인하고 로그인된 상태에서 요청을 제출하세요.',
          body2:
            '로그인할 수 없다면 지원팀에 연락해 계정 삭제 흐름으로 이동할 수 있도록 도움을 받으세요.',
        },
        restore: {
          title: '30일 복원 기간',
          body1:
            '삭제를 예약한 후 30일 이내에 다시 로그인하면 계정을 복원할 수 있습니다.',
          body2:
            '기한 전에 복원하지 않으면 삭제가 최종 완료되고 회원 프로필 접근이 종료됩니다.',
        },
        removed: {
          title: '삭제되는 항목',
          body1:
            '삭제가 최종 완료되면 계정 프로필, 회원 정보, 저장한 이벤트, 예약 및 프라이빗 이벤트 요청 기록, 그리고 계정에 연결된 회원 전용 접근 권한 또는 혜택이 삭제됩니다.',
          body2:
            '연결된 로그인 접근은 제공업체가 지원하는 범위 내에서 철회됩니다.',
        },
        retained: {
          title: '보관될 수 있는 항목',
          body1:
            '법적 준수, 사기 또는 오남용 방지, 보안 유지, 회계 처리, 또는 삭제 요청이 처리되었음을 기록하기 위해 필요한 제한된 기록은 보관될 수 있습니다.',
          body2: '이러한 기록은 해당 목적에 필요한 기간 동안만 보관됩니다.',
        },
      },
    },
    terms: {
